...
def follow_instruction(stacks, instruction):
    ...
    crates = 1
    while crates <= int(instruction_list[1]):
        # Pop the crate off its current stack and insert it at top of destination stack
        crate_to_move = stacks[int(instruction_list[3])-1].pop(0)
        stacks[int(instruction_list[5])-1].insert(0, crate_to_move)
        crates += 1
